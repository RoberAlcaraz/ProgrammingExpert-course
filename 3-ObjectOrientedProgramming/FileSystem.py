import os


class FileSystem:
    def __init__(self):
        self.root = Directory(
            "C:/Users/Rob/Documents/ProgrammingExpert-course")

    def create_directory(self, path):
        try:
            os.mkdir(path)
            print(os.path.dirname(os.path.realpath(path)))
        except Exception:
            raise ValueError()

    def create_file(self, path, contents):
        try:
            file = open(path, "w")
            file.write(contents)
            file.close()
        except FileNotFoundError:
            raise ValueError()

    def read_file(self, path):
        try:
            file = open(path, 'r')
            strng = file.read()
            file.close()
            return strng
        except Exception:
            print('hi')
            raise ValueError()

    def delete_directory_or_file(self, path):
        if os.path.isdir(path):
            os.removedirs(path)
        elif os.path.isfile(path):
            os.remove(path)

    def size(self):
        txt_files = []

        for dir in os.listdir(self.root.name):
            if dir.endswith('.txt'):
                txt_files.append(os.path.join(self.root.name, dir))
            for root, _, files in os.walk(dir):
                for file in files:
                    if file.endswith('.txt'):
                        txt_files.append(os.path.join(
                            self.root.name, root.replace('\\', '/'), file))

        n_characters = 0
        for file_path in txt_files:
            file = open(file_path, 'r')
            n_characters += len(file.read())
            file.close()

        return n_characters

    def __str__(self):
        return f"*** FileSystem ***\n" + self.root.__str__() + "\n***"

    @staticmethod
    def _validate_path(path):
        if not path.startswith("/"):
            raise ValueError("Path should start with `/`.")

    def _find_bottom_node(self, node_names):
        # Write your code here.
        pass


class Node:
    def __init__(self, name):
        self.name = name

    def __str__(self):
        return f"{self.name} ({type(self).__name__})"


class Directory(Node):
    def __init__(self, name):
        super().__init__(name)
        self.children = {}

    def add_node(self, node):
        self.children[node.name] = node

    def delete_node(self, name):
        del self.children[name]

    def __str__(self):
        string = super().__str__()

        children_strings = []
        for child in list(self.children.values()):
            child_string = child.__str__().rstrip()
            children_strings.append(child_string)

        children_combined_string = indent("\n".join(children_strings), 2)
        string += "\n" + children_combined_string.rstrip()
        return string


class File(Node):
    def __init__(self, name):
        super().__init__(name)
        self.contents = ""

    def write_contents(self, contents):
        self.contents = contents

    def __len__(self):
        return len(self.contents)

    def __str__(self):
        return super().__str__() + f" | {len(self)} characters"


def indent(string, number_of_spaces):
    spaces = " " * number_of_spaces
    lines = string.split("\n")
    indented_lines = [spaces + line for line in lines]
    return "\n".join(indented_lines)


fs = FileSystem()
fs.create_file("tim.txt", "12345")
print(fs.size())
# fs.create_directory("dir1")
# fs.create_directory("dir1/dir2")
# fs.create_directory("dir1/dir2/dir3")
# fs.create_file("dir1/dir2/file1.txt", "1")
# fs.create_file("dir1/dir2/dir3/file2.txt", "1")
# print(fs.size())
